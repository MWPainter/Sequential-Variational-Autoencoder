    ...
    def visualize(self, epoch, batch_size=10, use_gui=True):
        ...
        for i in range(2):
            ...
            if self.add_noise_to_chain:
                ...
                v = np.zeros([z[0].shape[0] * self.data_dims[0] * 2, len(z) / 2 * self.data_dims[1], self.data_dims[2]])
            else:
            ...
            for b in range(0, z[0].shape[0]):
                for t in range(0, len(z)):
                    if self.add_noise_to_chain and t <= ((len(z) + 1) / 2):
                        v[2*b*self.data_dims[0]:(2*b+1)*self.data_dims[0], t*self.data_dims[1]:(t+1)*self.data_dims[1]] = \
                                                                                        self.dataset.display(z[t][b])
                    elif self.add_noise_to_chain: # and t > ((len(z) + 1) / 2):
                        v[(2*b+1)*self.data_dims[0]:(2*b+2)*self.data_dims[0], t*self.data_dims[1]:(t+1)*self.data_dims[1]] = \
                                                                                        self.dataset.display(z[t][b])
                    else:
